...
const Login = () => {
  ...
  const onSubmitHandler = async (event) => {
    event.preventDefault();       //prevents reloading when form is submitted
    try {
  ...
  return (
    <form onSubmit={onSubmitHandler} action="" className="flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800">
      ...
      <button className='bg-black rounded-sm text-white font-light px-8 py-2 mt-4'>{currentState === 'Login' ? 'Sign In' : 'Sign Up'}</button>
    </form>
  )
}
